...
| **构建工具** | Vite | 4.5.0 |
| **数据存储** | BoltDB (区块监听) + CouchDB (Fabric) | - |
...
| 成员 | 角色 | 主要负责 |
|------|------|----------|
| **宋杨辉** | 主开发工程师 | 核心链码（5个合约）、后端API（农业局+检测中心）、Fabric SDK、前端（农业局+检测中心页面）、网络部署 |
| **卢世豪** | 文档工程师 | PPT制作、技术文档、链码（物流合约+溯源合约）、后端API（供应链平台）、前端（首页+供应链+链上可视化） |

> 详细分工见 [`云岭天眼_技术文档.md`](./云岭天眼_技术文档.md#12-项目分工) 第12章
